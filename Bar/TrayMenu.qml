...
PanelWindow {
    ...
    Item {
        ...
        // Center on the clicked tray icon's y, clamped so the panel never
        // runs off the top/bottom of the screen.
        anchors.topMargin: Math.max(
            Config.gap.sm,
            Math.min(trayMenu.targetY - height / 2, parent.height - height - Config.gap.sm)
        )
